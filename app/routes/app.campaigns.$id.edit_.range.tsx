...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  // Declare config before using it in Promise.all
  ...
  const [collections, productMeta, prefilledProducts, prefilledExcludedProducts, prefilledCollections] =
    await Promise.all([
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (selectionMode === "products" && selectedProducts.length === 0) {
    ...
    if (existingCount === 0) errors.products = es.nuevaRango.errProductos;
  }
  ...
  if (shouldActivate) {
    ...
    if (hasNewProducts || isAllStore || hasNewCollections || hasNewTags || hasNewVendors || hasNewTypes) {
      ...
      try {
        const result = await applyRangeDiscount(admin, campaignId, {
          mode,
          value,
...
export default function EditRangeCampaign() {
  ...
  const [mode, setMode] = useState<RangeMode>(campaign.mode);
  ...
  const [selectedProducts, setSelectedProducts] = useState(campaign.prefilledProducts);
  const [selectedCollections, setSelectedCollections] = useState(campaign.prefilledCollections);
  ...
  const [excludedProducts, setExcludedProducts] = useState(campaign.prefilledExcludedProducts);
  ...
  return (
    <s-page heading={es.editarRango.titulo}>
      ...
      <Form method="post">
        ...
        >
          <div>
              ...
              >
                ...
                <div>
                  {selectionMode === "products" && (
                    ...
                    >
                      {campaign.existingProductsCount > 0
                        ? es.editarRango.reemplazarSeleccion
              ...
              {selectionMode === "products" && (
                <div style={{ marginTop: "10px" }}>
                  {campaign.existingProductsCount > 0 && selectedProducts.length === 0 && (
                    ...
                    >
                      ✓ {campaign.existingProductsCount} {es.editarRango.productosActuales}
                    </div>
                  )}
                  <ProductChips
            ...
            {/* ─── 4. Programar campaña ─── */}
            <Section title={es.nuevaRango.secProgramacion} defaultOpen>
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "1fr 1fr",
